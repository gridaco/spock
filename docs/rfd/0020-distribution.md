...
The package is produced by a **small hand-rolled GitHub Actions workflow**,
...
it via OIDC; a verify job installs the published package on
macOS/Linux/Windows and runs it. No `dist`, no GitHub Release, no installers,
...
| `spock new` / `spock init` scaffolding | **framework (P1)** | Create a canonical project or adopt existing sources without a checkout. |
...
dispatch / tag vX.Y.Z
 ...
 │    ├─ assemble npm/binaries/<key>/ (+chmod +x on unix)
 │    ├─ guard: all four platforms and the sidecar are present
 │    ├─ stamp the resolved version into package.json
 │    ├─ npm pack: exact file set, exact 0755 executables, tarball ≤ 25 MiB
...
and measures the actual artifact against the framework's 25 MiB budget.

**Version.** CI derives the version from `[workspace.package]`; a tag or an
optional dispatch value is an assertion, never an independent source. It then
stamps `package.json`, so the npm version always equals the Rust build it
wraps. The former `spock` reservation stub was grown in place
...
- [ ] Run the first framework release dry run, then verify published framework
      routes and the installed sidecar in addition to the historical file
      command and embedded Studio smoke tests.
...
- **Missing or mismatched framework sidecar (medium unguarded → low with the
  ...
  exact commits and protocols to every hashed file, `npm pack` proves the tree
  is published, and the cross-OS verify job rechecks the installed bytes before
  exercising framework routes.
