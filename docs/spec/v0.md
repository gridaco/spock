...
scalar type instead of a shape). A fn's `sql` body travels in the
...
**Engine builtins.** The runtime registers two SQL functions on the
connection before DDL: `spock_uuid()` (UUIDv7) and `spock_now()`
(RFC 3339 UTC) — *the same generators the write path uses* for `auto`
and `now`. Consequences: an escape body (§7.4) may omit defaulted
columns from an INSERT and receive the engine's own values via the
DEFAULT clauses, or call the builtins directly; a value minted inside
the escape is indistinguishable in format and id-version from one the
engine minted. Hand-rolling ids or timestamps in escape SQL is never
necessary.
...
- the statement's **result columns equal the declared return shape's
  ...
  DML must use `RETURNING`. A **scalar return** (`-> int` and friends)
...
rows (`-> int` → a number, `-> [text]` → an array of strings). A SQL
`NULL` value renders as JSON `null`: for `-> t?` a missing row and a
NULL value are indistinguishable by design; for an un-suffixed scalar a
NULL is the body's own lie surfacing truthfully — the checker cannot see
values (the body is `unchecked`).
