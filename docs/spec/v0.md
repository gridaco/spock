...
> non-normative.** The `0.5.2` toolchain accepts top-level `error`
> declarations and carries the proposed specification text below so the RFD
> 0024 prototype can be reviewed against an exact target. RFD 0024 remains
...
> acceptance. RFD 0012's implicit per-function refusal minting remains the
> normative v0 rule even though the `0.5.2` implementation preview deliberately
> replaces it. The proposed text may become normative only after acceptance
> and full implementation reconciliation under the language-change process.
...
marked below are the **experimental, non-normative 0.5.2 implementation
preview** described in the opening notice.
...
`error` is active in the experimental `0.5.2` implementation preview. It
remains reserved in the normative v0 language while that RFD is draft.
...
- `///` — an **outer** doc comment: documents the entity that *follows* it (a
  ...
  `error` is part of the experimental RFD 0024 implementation preview.
...
In the experimental RFD 0024 implementation preview, `error ident` declares one authored
...
- Under the experimental RFD 0024 implementation preview, a product-error declaration and
...
undocumented. The experimental RFD 0024 implementation preview adds top-level `errors`, an
...
protocol-reserved errors, and—in the experimental RFD 0024 implementation preview—explicitly
...
raised from a fn body (§7.4). In the experimental RFD 0024 implementation preview, the code is
...
declare still surface truthfully. Under the experimental RFD 0024 implementation preview,
